...
%   data - column or row data
%   vDeadband - deadband for determining what's a peak and what's not

% Normalize the data
preAverage = mean(data);
data = data - preAverage;
...
[~, order] = sort(maximin(:,1));    % Get chronological order of combined peaks and troughs
maximin = maximin(order,:);         % Rearrange
