...
for i = 1:size(SA,1)-1
    ...
    Vlin(i+1) =   Vlin(i)*(1-(dt/(Rl*C))) ...
               + (dt/C)*(2*pi*f*dq*cos(2*pi*f*time(i)));
...
%     subplot(2,1,1);
%     [ax,~,~] = plotyy( time, SA, time, v);%;, time, -1.*Vlin);%time(1:end-1), v(1:end-1).*dqdt );
%     hold(ax(2));
%     plot(ax(2),time,-1.*Vlin);
%     title('Voltage');
%     legend('Surface Area','Non-Linear Model Response','Linear Model Response');
%     subplot(2,1,2);
%     plot(time,q);
%    waitforbuttonpress();
    subPlotModified( time, SA, v, -1.*Vlin, '', 'Time (s)','Surface Area (mm^2)', ...
